...
class AuthControllerTest {
  ...
  void login_Success() throws Exception {
    ...
    mockMvc
        .perform(
            post(TestConstants.LOGIN_ENDPOINT)
                .with(csrf())
  ...
  void login_InvalidCredentials() throws Exception {
    ...
    mockMvc
        ...
        .andExpect(
            jsonPath(TestConstants.JSON_MESSAGE).value("Authentication failed: Bad credentials"))
  ...
  void register_Success() throws Exception {
    ...
    mockMvc
        .perform(
            post(TestConstants.REGISTER_ENDPOINT)
                .with(csrf())
                .contentType(MediaType.APPLICATION_JSON)
        ...
        .andExpect(jsonPath(TestConstants.JSON_SUCCESS).value(true))
        .andExpect(
            jsonPath(TestConstants.JSON_MESSAGE).value(TestConstants.REGISTRATION_SUCCESS_MESSAGE))
        .andExpect(jsonPath("$.userId").value(1))
        .andExpect(jsonPath("$.firstName").value(TestConstants.TEST_FIRST_NAME))
        .andExpect(jsonPath("$.lastName").value(TestConstants.TEST_LAST_NAME))
        .andExpect(jsonPath("$.email").value(TestConstants.TEST_EMAIL));
  ...
  void register_UserAlreadyExists() throws Exception {
    ...
    when(userService.registerUser(any(UserRegistrationRequest.class)))
        .thenThrow(
            new UserAlreadyExistsException(
                "User already exists with email: " + TestConstants.TEST_EMAIL));
    ...
    mockMvc
        ...
        .andExpect(jsonPath(TestConstants.JSON_SUCCESS).value(false))
        .andExpect(
            jsonPath(TestConstants.JSON_MESSAGE)
                .value(
  ...
  void register_ServiceReturnsEmpty() throws Exception {
    // Given
    when(userService.registerUser(any(UserRegistrationRequest.class))).thenReturn(Optional.empty());
  ...
  void register_GeneralException() throws Exception {
    ...
    mockMvc
        ...
        .andExpect(
            jsonPath(TestConstants.JSON_MESSAGE)
                .value("Registration failed: Database connection failed"));
  }

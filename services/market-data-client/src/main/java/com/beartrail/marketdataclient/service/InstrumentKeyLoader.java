    ...
    try {
      try (var is =
          Thread.currentThread()
              ...
              .getResourceAsStream("instrument_keys.json")) {
        if (is == null) {
      ...
      if (EQUITY_SEGMENTS.contains(segment)) {
        ...
        if (!instrumentKey.isEmpty()) {
          ...
          String tradingSymbol =
              instrumentNode.has("trading_symbol")
                  ? instrumentNode.get("trading_symbol").asText()
                  : "N/A";
          String name = instrumentNode.has("name") ? instrumentNode.get("name").asText() : "N/A";

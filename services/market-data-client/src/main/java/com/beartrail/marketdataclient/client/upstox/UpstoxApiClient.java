...
public class UpstoxApiClient {
  ...
  public List<PriceUpdateEvent> getPriceUpdateEvents(List<String> symbolList, String interval) {
    String url =
        String.format(
            "%s/market-quote/ohlc?instrument_key=%s&interval=%s",
            baseUrl, String.join(",", symbolList), interval);
    ...
    ResponseEntity<LatestMarketDataResponseDto> responseEntity =
        restTemplate.exchange(url, HttpMethod.GET, entity, LatestMarketDataResponseDto.class);
